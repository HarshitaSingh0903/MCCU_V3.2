...
class QTextEditLogger(logging.Handler):
    def __init__(self, parent):
        ...
        self.widget.setStyleSheet( """QPlainTextEdit {background-color: #303;  color: #00FF00; font-family: Courier;}""" )
...
class UI(QMainWindow):
    ...
    def __init__(self):
        ...
        uic.loadUi( os.path.join(os.path.dirname(os.path.abspath(__file__)), "ui//main_win.ui"),  self, )
        self.i = 0
        self.logCounter=0
        ...
        validators = [
            (self.satBfAdd, double_validator),(self.satLatAdd, double_validator), (self.satLonAdd, double_validator),
            (self.satPolAdd, double_validator), (self.satTwoLineAdd, double_validator), (self.azAngleInput, double_validator), 
            (self.elAngleInput, double_validator), (self.xelAngleInput, double_validator), (self.polAngleInput, double_validator),
            (self.azDirAngleInput, double_validator), (self.elDirAngleInput, double_validator), (self.xelDirAngleInput, double_validator),
            (self.polDirAngleInput, double_validator),  (self.sysTempMax, double_validator), (self.sysTempMin, double_validator),
            (self.rollMin, double_validator),  (self.rollMax, double_validator), (self.pitchMin, double_validator), 
            (self.pitchMax, double_validator), (self.yawMin, double_validator), (self.yawMax, double_validator),
            (self.beaconMin, double_validator), (self.beaconMax, double_validator), (self.tarAzMin, double_validator),
            (self.tarAzMax, double_validator), (self.tarElMin, double_validator), (self.tarElMax, double_validator),
            (self.tarPolMin, double_validator), (self.tarPolMax, double_validator), (self.currElMin, double_validator),
            (self.currElMax, double_validator), (self.currxElMin, double_validator), (self.currxElMax, double_validator),
            (self.currAzMin, double_validator), (self.currAzMax, double_validator), (self.currPolMin, double_validator),
            (self.currPolMin, double_validator), (self.mCurrElMin, double_validator), (self.mCurrElMax, double_validator),
            (self.mCurrXElMin, double_validator), (self.mCurrXElMax, double_validator), (self.mCurrAzMin, double_validator),
            (self.mCurrAzMax, double_validator), (self.mCurrPolMin, double_validator), (self.mCurrPolMax, double_validator),
            (self.bucCurrMin, double_validator), (self.bucCurrMax, double_validator), (self.ofcCurrMin, double_validator),
            (self.ofcCurrMax, double_validator), (self.slscCurrMin, double_validator), (self.slscCurrMax, double_validator),
            (self.totalCurrMin, double_validator), (self.totalCurrMax, double_validator),
            (self.ipSlsc1, int_validator), (self.ipSlsc2, int_validator), (self.ipSlsc3, int_validator), (self.ipSlsc4, int_validator),
        ...
        self.plotWidget_3.setGaugeTheme(2)
    ...
    #     doc = fitz.open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "mccuUserManual_v2.pdf"))
    ...
    def machineInfo(self):
        ...
        # for config in wmi_network_configs:
        #     for dns_server in config.DNSServerSearchOrder:
    ...
    def on_saveBtn_toggled(self):
        ...
        if ok and text:
            if text == "123":
                ...
                configParameter.editConfigFile(tempMinValue, tempMaxValue,rollMinValue, rollMaxValue, pitchMinValue, pitchMaxValue, yawMinValue, yawMaxValue,
    ...
    def beacon_scale_conversion(value, in_min, in_max, out_min, out_max):
            ...
            Returns:
                ...
                out_max
            """
            input_value = ((value - out_min) / (out_max - out_min)) * (in_max - in_min) + in_min
    ...
    def update(self):
        try:
            ...
            if len(self.data) > 0:
                ...
                # osu_list=[self.data[14],self.data[37], self.data[15],self.data[38], self.data[16],self.data[39],self.data[28]]
                # print('roll_1: ', self.data[14])
                # print('roll_2: ', self.data[37])
                # print('pitch_1: ', self.data[15])
                # print('pitch_2: ', self.data[38])
                # print('yaw_1: ', self.data[16],'yaw_2: ', self.data[39], 'az : ', self.data[28])
                # service.write_to_osuDB(osu_list)
                ...
                #>>>>>>>>.>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> ERROR BOX DASHBOARD 
                # if self.data[1]==0:
                #     self.osuErrorCounter+=1
                #     if self.osuErrorCounter==5:
                #         self.groupBox_13.show()
                #         self.osuErrorFlag=1
                #         self.errorStateAdd()
                    
                    # Assuming you already have groupBox_13
                #>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> manual mode set 
                ...
                converted_value = UI.beacon_scale_conversion(float(self.data[17]), 0,100,-80,-60)
            ...
            if self.ConnectingCounter % 5==0:
                ...
                self.ConnectingCounter=0
            self.icon_only_widget.setEnabled(False)
            self.stackedWidget.setEnabled(False)
            self.widget_19.setEnabled(False)
    ...
    def updateLogTable(self):
        ...
        for j in self.log:
            if r < 20:
                ...
                # self.dataLogTable.setItem(r, 24, QTableWidgetItem(str(j[24])))
                r += 1
            else:
                self.log.clear()
                r = 0
    ...
    def setManualParam(self):
        if len(self.azAngleInput.text()) == 0:
            self.azAngleInput.setText("0")
            self.azManualLog =f'azimuth angle:{self.azAngleInput.text()}'
            # print(self.azManualLog)
        else:
            self.azManualLog =f'azimuth angle:{self.azAngleInput.text()}'
            # print(self.azManualLog)
            
        if len(self.elAngleInput.text()) == 0:
            self.elAngleInput.setText("0")
            self.elManualLog =f'elevation angle:{self.elAngleInput.text()}'
            # print(self.elManualLog)
        else:
            self.elManualLog =f'elevation angle:{self.elAngleInput.text()}'
            # print(self.elManualLog)
        if len(self.xelAngleInput.text()) == 0:
            self.xelAngleInput.setText("0")
            self.xelManualLog =f'cross-elevation angle:{self.xelAngleInput.text()}'
            # print(self.xelManualLog)
        else:
            self.xelManualLog =f'cross-elevation angle:{self.xelAngleInput.text()}'
            # print(self.xelManualLog)
        if len(self.polAngleInput.text()) == 0:
            self.polAngleInput.setText("0")
            self.polManualLog =f'polarisation angle:{self.polAngleInput.text()}'
            # print(self.polManualLog)
        else:
            self.polManualLog =f'polarisation angle:{self.polAngleInput.text()}'
            # print(self.polManualLog)
        time_taken_az= UI.motorRampingTimeCalcy(float(self.azAngleInput.text()), 3.0, 20000)
        time_taken_el= UI.motorRampingTimeCalcy(float(self.elAngleInput.text()), 3.0, 20000)
        time_taken_xel= UI.motorRampingTimeCalcy(float(self.xelAngleInput.text()), 3.0, 20000)
        time_taken_pol= UI.motorRampingTimeCalcy(float(self.polAngleInput.text()), 2.88, 25000)
        ...
        azAngle= int((float(self.azAngleInput.text()))*100)
        elAngle = int((float(self.elAngleInput.text()))*100)
        xelAngle = int((float(self.xelAngleInput.text()))*100)
        polAngle = int((float(self.polAngleInput.text()))*100)
        azDir= int(self.azDirAngleInput.text())
        elDir= int(self.elDirAngleInput.text())
        xelDir= int(self.xelDirAngleInput.text())
        polDir= int(self.polDirAngleInput.text())
        self.manualLog= self.azManualLog + f'°({str(azDir)})' + ' / '+self.elManualLog + f'°({str(elDir)})' +' / '+self.xelManualLog + f'°({str(xelDir)})' +' / '+self.polManualLog + f'°({str(polDir)})'
        ...
        if not (azAngle>18000 or elAngle>5000 or xelAngle>5000 or polAngle>5000):
            ...
            if  (azDir<=1 and elDir<=1 and xelDir<=1 and polDir <=1):
                ...
                self.manualErrMsg.setText("Direction out of range. It can either be 1 or 0")
    ...
    def manualMode(self):
        ...
        if self.flag==1:
            if manualParameters!=None :
                ...
                self.manualErrMsg.setText("Angles or direction out of range")
                self.azTimeInput.setText(str( 0))
                self.elTimeInput.setText(str(0))
                self.xelTimeInput.setText(str(0))
                self.polTimeInput.setText(str(0))   
    ...
    def loadSatData(self):
        ...
        if file_exists == True:
            ...
            global header
            header = ( "Satellite Name", "Beacon Freq.", "Latitude",  "Longitude", "Polarization" , " Norad Sat info" )
            self.satelliteTable.setHorizontalHeaderLabels(header)
            row_index = 0
            for value_tuple in self.satinfo:
                col_index = 0
                for value in value_tuple:
                    self.satelliteTable.setItem(
    ...
    def addSatRow(self):
        ...
        if file_exists:
            ...
            if len(satName)==0 and len(satBF)==0 and len(satLon)==0 and len(satLat)==0 and len(satPol) and len(satNomadTwoLine)==0:
    ...
    def editSatRow(self):
        ...
        satFileHandling.edit_row_in_csv(current_Row, [editedSatName,  editedBF, editedLat, editedLon,editedPol, editedNorad])
        UI.loadSatData(self)
        self.addEditSatFrame.hide()
        self.satelliteTable.setCurrentCell(-1, -1)
        self.last_selected_row = -1  # Reset the last selected row
    ...
    def monitorGraph(self):
        self.upperGraphLayout2 = QtWidgets.QHBoxLayout(self.upperGraph)
        self.upperGraphLayout2.setObjectName("horizontalLayout2")
        self.lowerFrameLayout3 = QtWidgets.QHBoxLayout(self.lowerFrame)
        self.lowerFrameLayout3.setObjectName("horizontalLayout3")

        self.axis_temp = ColoredAxis( orientation="left", axisPen=pg.mkPen(color="black", width=1) )
        self.axis_sat = ColoredAxis( orientation="left", axisPen=pg.mkPen(color="black", width=1) )
        self.timeAxis = []

        self.tempSlsc = []
        self.tempGraphWidget = pg.PlotWidget( axisItems={"bottom": pg.DateAxisItem(), "left": self.axis_temp} )
        self.tempPlot = self.tempGraphWidget.plot( self.timeAxis, self.tempSlsc, pen=pg.mkPen("g", width=2)  )
        self.tempGraphWidget.showGrid(x=True, y=True)
        self.tempGraphWidget.setBackground("w")
        self.tempGraphWidget.setLabel( "bottom", '<p style="font-size:15px;color:green">Time</p>'  )
        self.tempGraphWidget.setLabel(  "left", '<p style="font-size:15px;color:green">Temperature(°)</p>' )
        self.tempGraphWidget.setTitle( '<p style="font-size:20px;color:green">Temperature</p>' )
        self.tempGraphWidget.getAxis("bottom").setTickFont(  QtGui.QFont("Arial", 10, QFont.Bold)  )
        self.tempGraphWidget.getAxis("left").setTickFont( QtGui.QFont("Arial", 10, QFont.Bold) )
        ...
        self.osuRollPlot = self.osuGraphWidget.plot( self.timeAxis, self.osuRoll, pen=pg.mkPen("violet", width=2), symbol="+",
            symbolSize=10, symbolBrush="b", name="ROLL", )
        self.osuPitchPlot = self.osuGraphWidget.plot( self.timeAxis, self.osuPitch, pen=pg.mkPen("r", width=2),  symbol="p",
            symbolSize=3, symbolBrush="y", name="PITCH", )
        self.osuYawPlot = self.osuGraphWidget.plot( self.timeAxis,  self.osuYaw, pen=pg.mkPen("green", width=2), symbol="d",
            symbolSize=3, symbolBrush="r", name="YAW", )
        ...
        self.osuGraphWidget.setTitle( '<p style="font:bold;font-size:20px;color:black">OSU</p>' )
        ...
        self.lowerFrameLayout3.addWidget(self.tempGraphWidget)
    ...
    def updateGraph(self):
        ...
        if self.i % 10 == 0:
            ...
            if ( (len(self.tempSlsc)) <= 60 and (len(self.osuRoll)) <= 60 and (len(self.osuPitch)) <= 60 and (len(self.osuYaw)) <= 60):
                self.tempSlsc.append(float(self.data[13]))
                ...
                self.osuYaw.append(float(self.data[16]))
            self.systemMsg.setText(" ")
            self.tempPlot.setData(self.timeAxis, self.tempSlsc)
    ...
    def check_checkbox_state(self):
        if ( self.rollCheckBox.isChecked() and self.pitchCheckBox.isChecked() and self.yawCheckBox.isChecked() ):
            ...
            try:
                ...
                print("NO CHANGE")
        elif ( self.rollCheckBox.isChecked() and self.pitchCheckBox.isChecked() and not self.yawCheckBox.isChecked() ):
            ...
            try:
                ...
                print("NO CHANGE")
        elif ( self.pitchCheckBox.isChecked() and self.yawCheckBox.isChecked() and not self.rollCheckBox.isChecked() ):
            # print("pitch and yaw is checked")
            self.osuGraphWidget.removeItem(self.osuRollPlot)
            self.osuGraphWidget.addItem(self.osuYawPlot)
            self.osuGraphWidget.addItem(self.osuPitchPlot)
        elif ( self.yawCheckBox.isChecked() and self.rollCheckBox.isChecked() and not self.pitchCheckBox.isChecked() ):
            # print("yaw and roll is checked")
            self.osuGraphWidget.removeItem(self.osuPitchPlot)
            self.osuGraphWidget.addItem(self.osuYawPlot)
            self.osuGraphWidget.addItem(self.osuRollPlot)
        elif ( self.rollCheckBox.isChecked() and not self.pitchCheckBox.isChecked() and not self.yawCheckBox.isChecked() ):
            # print("roll is checked")
            self.osuGraphWidget.removeItem(self.osuYawPlot)
            self.osuGraphWidget.removeItem(self.osuPitchPlot)
            self.osuGraphWidget.addItem(self.osuRollPlot)
        elif ( self.pitchCheckBox.isChecked() and not self.yawCheckBox.isChecked() and not self.rollCheckBox.isChecked() ):
            # print("pitch is checked")
            self.osuGraphWidget.removeItem(self.osuYawPlot)
            self.osuGraphWidget.removeItem(self.osuRollPlot)
            self.osuGraphWidget.addItem(self.osuPitchPlot)
        elif ( self.yawCheckBox.isChecked() and not self.rollCheckBox.isChecked() and not self.pitchCheckBox.isChecked() ):
    ...
    def diagnostics(self):
        ...
        # AZ current check
        if ( config["motorAzCurr"]["minimum"] <= float(self.data[31]) <= config["motorAzCurr"]["maximum"] ):
            self.adeAzMotorBtn.setStyleSheet( safe )
        else:
            self.adeAzMotorBtn.setStyleSheet( danger )
        # EL current check
        if ( config["motorElCurr"]["minimum"] <= float(self.data[29]) <= config["motorElCurr"]["maximum"] ):
            self.adeElMotorBtn.setStyleSheet( safe )
        else:
            self.adeElMotorBtn.setStyleSheet( danger )
        # CROSS EL  current check
        if ( config["motorClCurr"]["minimum"] <= float(self.data[30]) <= config["motorClCurr"]["maximum"] ):
            self.adexElMotorBtn.setStyleSheet(safe )
        else:
            self.adexElMotorBtn.setStyleSheet(danger )
        # POL current check
        if ( config["motorPolCurr"]["minimum"] <= float(self.data[32]) <= config["motorPolCurr"]["maximum"] ):
        ...
        if ( config["sysTemp"]["minimum"] <= float(self.data[13]) <= config["sysTemp"]["maximum"] ):
    ...
    def modelStatus(self):
        ...
        if ( config["motorAzCurr"]["minimum"] <= float(self.data[32]) <= config["motorAzCurr"]["maximum"] ):
        ...
        if ( config["motorClCurr"]["minimum"] <= float(self.data[31]) <= config["motorClCurr"]["maximum"] ):
        ...
        if ( config["motorPolCurr"]["minimum"] <= float(self.data[30]) <= config["motorPolCurr"]["maximum"] ):
            self.polMotorStatus.setStyleSheet(safe)
        else:
            # print('pol :',self.data[30])
            self.polMotorStatus.setStyleSheet(danger)
        # BUC CURRENT STATUS
        if ( config["bucCurr"]["minimum"] <= float(self.data[33]) <= config["bucCurr"]["maximum"] ):
    ...
    #         else:
    #             homePosStatus= service.releaseHomePosition()
    #             if homePosStatus == 1:
    #                 logging.info("home position released")
    #                 self.systemMsg.setText("home position released")
    #             else:
    #                 self.systemMsg.setText("Home position not released")
    def homePos(self):
        if self.flag==1:
            homePosStatus= service.setHomePosition()
            if homePosStatus == 1:
    ...
    # def Safemode(self):
    #     if self.flag==1:
    #         if self.safeModeBtn1.isChecked():
    ...
    #         else:
    #             safeModeStatus= service.releaseSafemode()
    #             if safeModeStatus == 1:
    #                 logging.info("SafeMode Released")
    #                 self.systemMsg.setText("SafeMode Released")
    #             else:
    #                 self.systemMsg.setText("safeMode is not released")
    def Safemode(self):
        if self.flag==1:
            safeModeStatus= service.setSafemode()
            if safeModeStatus == 1:
    ...
    def restartSlsc(self):
        if self.flag==1:
            text, ok = QInputDialog.getText(None, "Security Password", "<b><font size='3'>To restart SLSC , enter security Password.", QLineEdit.Password, )
            if ok and text:
                if text == "123":
                    service.setRestart()
    ...
    def shutDownSlsc(self):
        if self.flag==1:
            text, ok = QInputDialog.getText(None, "Security Password", "<b><font size='3'>To shutdown SLSC, enter security Password.</font></b>", QLineEdit.Password)
            if ok and text:
                if text == "123":
                    service.setShutdown()
    ...
    def on_dashboardBtn2_toggled(self):
        self.stackedWidget.setCurrentIndex(0)

    def on_satBtn1_toggled(self):
        self.stackedWidget.setCurrentIndex(1)

    def on_satBtn2_toggled(self):
        self.stackedWidget.setCurrentIndex(1)

    def on_antennaBtn1_toggled(self):
        self.stackedWidget.setCurrentIndex(2)
        self.azDirAngleInput.setText('0')
        self.elDirAngleInput.setText('0')
        self.xelDirAngleInput.setText('0')
        self.polDirAngleInput.setText('0')

    def on_antennaBtn2_toggled(self):
        self.stackedWidget.setCurrentIndex(2)

    def on_monitorBtn1_toggled(self):
        self.stackedWidget.setCurrentIndex(3)

    def on_monitorBtn2_toggled(self):
        self.stackedWidget.setCurrentIndex(3)

    def on_diagBtn1_toggled(self):
        self.stackedWidget.setCurrentIndex(4)

    def on_diagBtn2_toggled(self):
        self.stackedWidget.setCurrentIndex(4)

    def on_logBtn1_toggled(self):
        self.stackedWidget.setCurrentIndex(5)

    def on_logBtn2_toggled(self):
        self.stackedWidget.setCurrentIndex(5)
    ...
    def motorRampingTimeCalcy(Axis_Ang, GR, PPR):
        ...
        if Axis_Ang>0 and Axis_Ang <=5:
            ...
            rpm_min=0.35
            rpm_max=3.0
        elif Axis_Ang>60 and Axis_Ang <=180:
            rpm_min=0.35
            rpm_max=3.75
        else:
            rpm_min=0.5
        ...
        if not Npulses==0:
            ...
            freq_S_curve=(fmax-fmin)*(np.sin(np.pi/2*t_ang/Fixed_S_Pulses))**S_exp+fmin
            ...
            S_dt_up=(2*np.sqrt(fmin)*np.sqrt(fmax)*Fixed_Start_Pulses/(2*fmin**2+2*(fmax-fmin)*fmin)*1000+1/((fmax-fmin)/8+fmin)*1000)/1000

            S_dt_Cons=((Npulses-Fixed_Start_Pulses-Fixed_Stop_Pulses)*1/fmax*1000)/1000
            S_dt_down=(2*np.sqrt(fmin)*np.sqrt(fmax)*Fixed_Stop_Pulses/(2*fmin**2+2*(fmax-fmin)*fmin)*1000+1/((fmax-fmin)/8+fmin)*1000)/1000
...
class CustomMessageDialog(QDialog):
    def __init__(test):
        ...
        test.setWindowFlags(test.windowFlags() | Qt.FramelessWindowHint)  # Remove window frame
