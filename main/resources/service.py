...
def connectToSlsc():
    ...
    main_socket_server = socket.socket()
    main_socket_server.settimeout(0.2)
    try:
...
def getUpdateData(): # 161,1 
    ...
    try:
        ...
        return result_list
        # if (len(slscRecvData)) >= 2:
        #     write_to_slscStatusLog(slscRecvData)
        #     return slscRecvData
        # else:
        #     print("data not found")
        #     return [slscRecvData]
...
class commands:
    def __init__(self) -> None:
        ...
        self.mc= [ord('M'),ord('C')] # [0x4d,0x43]
        self.vr = [ord('V'),ord('R')] # [0x56,0x52]
        self.li = [ord('L'),ord('I')] #[0x4C, 0x49] # ip command
        self.crlf = [ord('\r'),ord('\n')] #[0x0D, 0x0A]
        self.lp = [ord('L'),ord('P')] # [0x4C, 0x50] # port change
        self.st = [ord('S'),ord('T')] # [0x4C, 0x50]
        self.ps = [ord('P'),ord('S')] # DATA SIZE 
        self.save= [ord('S'),ord('V')] # [0x53, 0x56]
        self.reset= [ord('R'),ord('T')] # [0x52, 0x54]
        self.tr= [ord('T'),ord('R')] # [0x54, 0x52]
        self.mac = [0x00, 0x08, 0xDC, 0x69, 0x40, 0xD3]
        self.gw =  [ord('G'),ord('W')]
        self.ds = [ord('D'),ord('S')]
        self.sm = [ord('S'),ord('M')]
    ...
    def broadcastSetIp(self,newIp, newPort, subnetMask, gatway, dnsServer):
        ...
        lst = [self.ma, self.mac, self.crlf, self.pw, self.crlf,self.sm, self.sbMaskSet,self.crlf, self.gw, self.gatewaySet, self.crlf,
               self.ds, self.dnsServerSet, self.crlf,self.li, self.ip, self.crlf, self.lp, self.port_change,
               self.crlf,self.ps, self.data_size, self.crlf, self.save, self.crlf, self.reset, self.crlf]
