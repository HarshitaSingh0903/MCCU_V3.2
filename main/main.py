...
def checkIfFileExists():
    ...
    if satDBExists == False:
        # sat1= ("GSAT-15",11697.50,0,25,90,"1 41028U 15065A   23176.33313058 -.00000243  00000+0  00000+0 0  9995,2 41028   0.0114 346.6174 0003767  95.6642  44.3444  1.00267187 27957")
        ...
        # sat3= ("GSAT-7",11697.50,0,25,90,"1 41028U 15065A   23176.33313058 -.00000243  00000+0  00000+0 0  9995,2 41028   0.0114 346.6174 0003767  95.6642  44.3444  1.00267187 27957")
        satFileHandling.write_csv_without_header("satelliteDB.csv")
        # print("sat file created")
...
class LoginWidget(QWidget):
    def __init__(self):
        ...
        self.dnsInput_1.setValidator(int_validator)
        self.dnsInput_2.setValidator(int_validator)
        self.dnsInput_3.setValidator(int_validator)
        self.dnsInput_4.setValidator(int_validator)
        ...
        self.dnsInput_1.textChanged.connect(self.on_text_changed)
        self.dnsInput_2.textChanged.connect(self.on_text_changed)
        self.dnsInput_3.textChanged.connect(self.on_text_changed)
        self.dnsInput_4.textChanged.connect(self.on_text_changed)
        ...
        if text.isdigit():
            ...
            if sender == self.sbInput_1:
                ...
                if 0 <= number <= 255:
                    ...
                    self.ipConfigErrorMsg.setText("Octets must be between 0 and 255.")
            elif sender == self.dnsInput_1:
                if 0 <= number <= 255:
                    self.ipConfigErrorMsg.setText("")
                else:
                    self.ipConfigErrorMsg.setText("Octets must be between 0 and 255.")
            elif sender == self.dnsInput_2:
                if 0 <= number <= 255:
                    self.ipConfigErrorMsg.setText("")
                else:
                    self.ipConfigErrorMsg.setText("Octets must be between 0 and 255.")
            elif sender == self.dnsInput_3:
                if 0 <= number <= 255:
                    self.ipConfigErrorMsg.setText("")
                else:
                    self.ipConfigErrorMsg.setText("Octets must be between 0 and 255.")
            elif sender == self.dnsInput_4:
                if 0 <= number <= 255:
                    self.ipConfigErrorMsg.setText("")
                else:
                    self.ipConfigErrorMsg.setText("Octets must be between 0 and 255.")
            elif sender == self.avIpInput1:
                if 0 <= number <= 255:
                    self.ipConfigErrorMsg.setText("")
                else:
                    self.ipConfigErrorMsg.setText("Octets must be between 0 and 255.")
            elif sender == self.avIpInput2:
                if 0 <= number <= 255:
                    self.ipConfigErrorMsg.setText("")
                else:
                    self.ipConfigErrorMsg.setText("Octets must be between 0 and 255.")
            elif sender == self.avIpInput3:
                if 0 <= number <= 255:
                    self.ipConfigErrorMsg.setText("")
                else:
                    self.ipConfigErrorMsg.setText("Octets must be between 0 and 255.")
    ...
    def communicationWin(self):
        ...
        if (len(self.sbInput_1.text())!=0) and (len(self.sbInput_2.text())!=0) and (len(self.sbInput_3.text())!=0) and (len(self.sbInput_4.text())!=0):
            ...
            if (len(self.gatewayInput_1.text())!=0) and (len(self.gatewayInput_2.text())!=0) and (len(self.gatewayInput_3.text())!=0) and (len(self.gatewayInput_4.text())!=0):
                ...
                if (len(self.dnsInput_1.text())!=0) and (len(self.dnsInput_2.text())!=0) and (len(self.dnsInput_3.text())!=0) and (len(self.dnsInput_4.text())!=0):
                    self.dnsInput= self.dnsInput_1.text() +'.'+self.dnsInput_2.text()+'.'+self.dnsInput_3.text()+'.'+self.dnsInput_4.text()
                    if (len(self.avIpInput1.text())!=0 and len(self.avIpInput2.text())!=0) and (len(self.avIpInput3.text())!=0) and (len(self.avIpInput4.text())!=0) and (len(self.avPortInput.text())!=0):
                        if (self.gatewayInput_1.text()==self.dnsInput_1.text()==self.avIpInput1.text()) and (self.gatewayInput_2.text()==self.dnsInput_2.text()==self.avIpInput2.text()) and (self.gatewayInput_3.text()==self.dnsInput_3.text()==self.avIpInput3.text()):
                            command_instance = service.commands()
                            command_instance.broadcastSetIp(self.availableIp, self.availablePort, self.sbMask, self.gwInput, self.dnsInput)
                            # print(len(self.avIpInput1.text()))
                            self.funcWidget.setCurrentIndex(2)
                            ...
                            self.ipConfigErrorMsg.setText('Gateway , dns server or IP address Incorrect. Please check again')
                    else:
                        self.ipConfigErrorMsg.setText('Please fill ip and port  input ')
                else:
                    self.ipConfigErrorMsg.setText('Please fill DNS Server ')
            else:
...
if __name__ == "__main__":
    ...
    style_file = QFile(os.path.join(os.path.dirname(os.path.abspath(__file__)), "ui//style.qss"))
